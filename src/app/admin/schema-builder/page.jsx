        ...
        <div className="max-w-5xl mx-auto space-y-6 pb-16">
                                ...
                                try {
                                    ...
                                    if (data.success && data.sql) {
                                        navigator.clipboard.writeText(data.sql).then(() => {
                                            alert('Auto-generated Schema SQL copied to clipboard!\n\nYou can paste it in the Supabase SQL editor or click "Execute" from the preview panel once you paste it.');
                                        });
                                    } else {
                                        alert('Failed to fetch backend schema SQL.');
                                    }
                                } catch (e) {
                                    alert('Error fetching SQL API');
                                }
                            }}
                            className="flex items-center gap-2 px-4 py-2.5 bg-slate-800 text-white text-sm font-semibold rounded-lg hover:bg-slate-700 transition-colors shadow-sm shrink-0"
                            title="Generate SQL from Zod configuration"
                        >
                            <Database className="w-4 h-4" /> Export Zod SQL
                        </button>
                        <button
                            type="button"
                            onClick={addTable}
            ...
            {tables.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-16 bg-white rounded-xl border border-dashed border-gray-300 text-center">
                    ...
                    <p className="text-sm font-medium text-gray-500">No visual tables defined</p>
                    <p className="text-xs text-gray-400 mt-1">Click "Add Table" to start designing visually.</p>
                </div>
            ) : (
            ...
            <div className="bg-gray-900 rounded-xl overflow-hidden shadow-lg border border-gray-700">
                ...
                <div className="flex items-center justify-between px-5 py-3 border-b border-gray-700 bg-gray-800">
                    <div className="flex items-center gap-2 text-gray-400">
                        ...
                        <span className="text-xs font-semibold uppercase tracking-wider">Visual Builder SQL</span>
