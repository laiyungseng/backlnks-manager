...
import { ChevronDown, ChevronRight, Droplet, CheckCircle, CheckCircle2, Lock, Unlock, Hash, Database, Globe, Target, Zap, BarChart, Clock } from 'lucide-react';
...
export default function PlacementCard({ project, representativeHash }) {
    ...
    const formatLanguages = () => {
        // Priority to child table if joined
            ...
            <div className="p-6">
                ...
                <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-6 mb-8">
                    <div className="space-y-2">
                        <div className="flex items-center flex-wrap gap-3">
                            ...
                            {(() => {
                                ...
                                const statusValue = project.status || 'IN PROGRESS';
                                let statusText = statusValue.toUpperCase();

                                if (isFinalized || statusValue === 'Finalized') {
                                    badgeClass = "emerald-100 bg-emerald-50 text-emerald-700 border-emerald-200";
                                    statusText = "FINALIZED";
                                } else if (statusValue === 'Completed' || allFulfilled) {
                                    badgeClass = "emerald-100 bg-emerald-50 text-emerald-700 border-emerald-200";
                        ...
                        <div className="flex flex-wrap items-center gap-4">
                            ...
                            <div className="flex items-center gap-1.5">
                                ...
                                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{project.vendors?.vendor_name}</span>
                            </div>
                            {project.owner && (
                                <div className="flex items-center gap-1.5">
                                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Owner: {project.owner}</span>
                                </div>
                            )}
                            <div className="flex items-center gap-1.5">
                                <Clock className="w-3.5 h-3.5 text-indigo-400" />
                                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Created: {project.created_date ? new Date(project.created_date).toLocaleDateString() : '—'}</span>
                            </div>
                            {project.completed_date && (
                                <div className="flex items-center gap-1.5">
                                    <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" />
                                    <span className="text-[10px] font-bold text-emerald-600 uppercase tracking-widest">Done: {new Date(project.completed_date).toLocaleDateString()}</span>
                                </div>
                            )}
                            {localLockState ? (
                    ...
                    <div className="flex items-end gap-3">
                        <VendorLinkCopy projectHash={representativeHash} vendorName={project.vendors?.vendor_name || 'unknown'} />
                        <button
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                    ...
                    <div className="bg-slate-50 rounded-lg p-3 border border-slate-100">
                        ...
                        <p className="text-xs font-black text-slate-800 uppercase tracking-tight">
                            {(project.country) || 'GLOBAL'} / {formatLanguages() || 'EN'}
                        </p>
                    </div>
                    ...
                    <div className="bg-slate-50 rounded-lg p-3 border border-slate-100">
                        ...
                        {(project.dripfeed_enabled) ? (
                            <p className="text-xs font-black text-amber-600 uppercase tracking-tight">
                                {project.urls_per_day} URLS/DAY {project.dripfeed_period && `(${project.dripfeed_period} DAYS)`}
                            </p>
                        ) : (
                    ...
                    <div className="bg-slate-50 rounded-lg p-3 border border-slate-100">
                        ...
                        <p className="text-xs font-bold text-slate-600 italic truncate">
                            {(() => {
                                const sheets = Array.isArray(project.project_targets)
                                    ? Array.from(new Set(project.project_targets.map(t => t.sheet_name).filter(Boolean)))
                                    : [];
