                    ...
                    <div className="px-4 py-5 sm:p-6">
                        ...
                        <dd className="mt-1 text-3xl font-semibold text-gray-900">
                            {projects?.filter(p => {
                                ...
                                return projectStatus === 'Completed' || projectStatus === 'Finalized' || hasPlacements;
                            }).length || 0}
                        </dd>
                    </div>
                </div>
                                    ...
                                    return (
                                        <tr key={project.id} className="hover:bg-gray-50 transition-colors">
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                {isEditMode ? (
                                                    <select value={project.project_targets?.[0]?.category || 'NULL'} onChange={(e) => handleFieldChange(project.id, 'backlinks_category', e.target.value)} className="w-24 px-1 py-1 border border-indigo-300 bg-white rounded text-xs select-auto">
                                                    ...
                                                    (() => {
                                                        const getCategories = () => {
                                                            const cat = project.project_targets?.[0]?.category;
                                                            return cat && cat !== 'NULL' ? cat : null;
                                                        };
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                {(() => {
                                                    ...
                                                    const isProjApproved = project.is_approved === true;

                                                    const isFinalized = projectStatus === 'Finalized' || hasPlacements;

                                                    let statusText = projectStatus || 'In Progress';
                                                    let badgeColor = 'bg-yellow-100 text-yellow-800';

                                                    if (isFinalized) {
                                                        statusText = 'Completed & Finalized';
                                                        badgeColor = 'bg-green-100 text-green-800';
                                                    } else if (projectStatus === 'Completed' || progressPercent === 100) {
                                                        statusText = 'Completed';
                                                        badgeColor = 'bg-green-100 text-green-800';
                                                    } else if (!isProjApproved) {
                                                        statusText = 'Inprocess-pending payment';
                                                        badgeColor = 'bg-amber-100 text-amber-800 border border-amber-200';
                                                    } else {
                                                        statusText = projectStatus || 'In Progress';
